import React from "react";
import style from "./styles.module.css";
import { MagnifyingGlass } from "phosphor-react";
import { useNavigate } from "react-router-dom";

export function Main() {
    const navigate = useNavigate()

    const item ={
        image: '/images/products/p4.jpg',
    }



    return (
        <div className={style.main}>
            <div className={style.search_container}>
                <input type='text' placeholder='pesquise por coleções'/>
                <button>
                    <MagnifyingGlass color='white' weight='fill' size={20}/>
                </button>
            </div>
            
            
            
            <div className={style.first_scroll_div}>
                {
                    [1,1,11,1,1,1,1,11,11].map(()=>{
                        return (
                            <div className={style.first_scroll_item} onClick={()=>navigate('/store', {state: item})}>
                                <img src="\images\products\p5.jpg"/>

                                <div className={style.store_top}>
                                    <img src='images\bg_hero.png' alt="" width='30px'/>
                                    <span>Eleza store</span>
                                </div>

                                <div className={style.desc_bottom}>
                                    <strong>Eleza Colection</strong>
                                    <p>#Summer with style</p>
                                </div>
                            </div>
                        )
                    })
                }
            </div>
        </div>
    );
}
